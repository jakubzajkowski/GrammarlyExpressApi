...
const TextTranslateController = async (req: Request, res: Response) => {
  try {
            ...
            if (user.prompts as number <= 1000){
              ...
              await User.findByIdAndUpdate({_id:_id},{prompts: (user.prompts as number)+1})
                const responseGrammar = await openai.chat.completions.create({
                    ...
                    return res.json({success:{text: responseGrammar.choices[0].message?.content}})
              }
              else{
...
export default TextTranslateController